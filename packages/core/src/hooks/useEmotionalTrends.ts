 ...
 *
 * Platform-agnostic hook that analyzes journal entries to detect:
...
export function useEmotionalTrends(): UseEmotionalTrendsReturn {
  ...
  const moodTrend = useMemo(() => {
    ...
    const previousData = moodData.filter(
    ...
    const previousAvg =
      previousData.reduce((sum, d) => sum + d.moodScore, 0) / previousData.length;
  ...
  const periods = useMemo((): EmotionalPeriod[] => {
    ...
    return Array.from(weeklyGroups.entries())
      .map(([weekKey, dataPoints]) => {
        const avgMood =
          dataPoints.reduce((sum, d) => sum + d.moodScore, 0) / dataPoints.length;
  ...
  const insights = useMemo((): EmotionalInsight[] => {
    ...
    if (moodTrend === 'improving') {
      result.push({
        ...
        description:
          'Your mood has been trending upward over the past week compared to the previous week.',
        significance: 0.8,
        actionable:
          'Consider noting what changes you made that might be contributing to this improvement.',
      });
      ...
      result.push({
        ...
        description:
          'Your mood has been trending downward over the past week. Be gentle with yourself.',
        significance: 0.9,
        actionable:
          'Consider reaching out to a friend, therapist, or trying a self-care activity.',
      });
    }

    // Positive correlations
    const positiveCorrelations = correlations.filter(
      (c) => c.isPositive && c.occurrences >= 3
    );
