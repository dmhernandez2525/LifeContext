...
export function JourneyImport({ onImport }: JourneyImportProps) {
  ...
  const handleFile = useCallback(async (file: File) => {
    try {
      ...
      if (!isJourneyFormat(text)) {
        setError('This file does not appear to be a Journey export. Expected a JSON file with journal entries.');
        setPhase('error');
        return;
      }
      const parsed = parseJourneyExport(text);
      ...
      setError('Failed to parse the file. Make sure it is a valid Journey JSON export.');
      setPhase('error');
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      <p className="text-sm font-medium text-gray-900 dark:text-white mb-1">
        Drop your Journey export here
      </p>
      <p className="text-xs text-gray-500 mb-4">
        Export from Journey: Settings &gt; Export &gt; JSON
      </p>
