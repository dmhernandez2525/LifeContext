    ...
    try {
      ...
      newAnniversaries.forEach(ann => {
        newPrompts.push({
          ...
          suggestedQuestion: "Looking back at this memory, what would you add or change about how you felt then?",
      ...
      categoryLastUsed.forEach((lastDate, prefix) => {
        if (lastDate < sixMonthsAgo && categoryNames[prefix]) {
          newPrompts.push({
            id: `gap-${prefix}`,
            type: 'gap',
            title: `Revisit ${categoryNames[prefix]}`,
            description: `You haven't explored ${categoryNames[prefix]} in over 6 months. Your perspective may have evolved.`,
            suggestedQuestion: `What's changed in your ${categoryNames[prefix].toLowerCase()} since you last reflected on it?`,
      ...
      if (recentJournals.length >= 3) {
        const moodCounts = recentJournals.reduce((acc, j) => {
          if (j.mood) {
        ...
        if ((moodCounts['low'] || 0) + (moodCounts['bad'] || 0) >= 3) {
          newPrompts.push({
            ...
            type: 'mood-based',
            title: 'Checking In',
            description: "Your mood has been lower than usual lately. Sometimes reflecting helps.",
            suggestedQuestion: "What's weighing on you right now? What would help lighten it?",
        ...
        if ((moodCounts['great'] || 0) >= 3) {
          newPrompts.push({
            ...
            description: "You've had several great days! Consider documenting what's working.",
            suggestedQuestion: "What's contributing to these good days? How can you maintain this momentum?",
      ...
      const unresolvedKeywords = [
        'need to', "haven't yet", 'want to', 'should', 'working on',
        'trying to', 'figure out', 'decide', 'considering'
      ];
          ...
          if (text.includes(keyword)) {
            ...
            if (relevantSentence) {
              newPrompts.push({
                ...
                type: 'follow-up',
                title: 'Following Up',
                description: `You mentioned something you were working on. How's it going?`,
                suggestedQuestion: `How has this situation evolved? Any progress or new insights?`,
